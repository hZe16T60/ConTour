...
// ========== Функція для створення гладкої кривої Catmull-Rom ==========
function catmullRomPath(points, closed = false) {
    ...
    if (closed) {
        // Замкнена крива: додаємо точки з зацикленням
        for (let i = 0; i < n; i++) {
            ...
            // Контрольні точки для кубічного безьє (еквівалент Catmull-Rom)
            ...
            const cp4 = { x: p2.x, y: p2.y };
            
            if (i === 0) {
                path.push(`M ${cp1.x} ${cp1.y}`);
            }
            path.push(`C ${cp2.x} ${cp2.y} ${cp3.x} ${cp3.y} ${cp4.x} ${cp4.y}`);
        }
        // Замикаємо (остання точка повертається до першої)
        ...
        // Відкрита крива: дублюємо першу та останню точки для дотичних
        for (let i = 0; i < n - 1; i++) {
...
function redrawCurrentMode() {
    ...
    const useCurve = smoothValue > 0; // Якщо >0, використовуємо гладкі криві
    ...
    if (mode === 'contour') {
        ...
        contours.forEach(contour => {
            ...
            contour.coordinates.forEach(polygon => {
                polygon.forEach(ring => {
                    ...
                    let d;
                    if (useCurve) {
                        // Визначаємо, чи замкнене кільце
                        const first = points[0];
                        const last = points[points.length - 1];
                        const closed = Math.hypot(first.x - last.x, first.y - last.y) < 1;
                        d = catmullRomPath(points, closed);
                    } else {
                        // Лінійна (оригінальна) полілінія
        ...
        console.log(`Додано path: ${pathCount} (згладжування кривими: ${useCurve})`);
...
function renderDisplacement(grid, nx, ny) {
    ...
    // Створюємо canvas високої роздільної здатності (1024x1024) з білінійною інтерполяцією
