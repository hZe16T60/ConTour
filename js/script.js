...
// Кастомна лінійка масштабу (виправлена)
...
function updateScaleBar() {
    ...
    let niceLength = 10;
    if (targetM > 1000) niceLength = 1000;
    else if (targetM > 500) niceLength = 500;
    else if (targetM > 200) niceLength = 200;
    else if (targetM > 100) niceLength = 100;
    else if (targetM > 50) niceLength = 50;
    else if (targetM > 20) niceLength = 20;
    else niceLength = 10;
    const actualPx = niceLength / metersPerPixel;
    const minWidth = 40;
    const finalWidth = Math.max(minWidth, actualPx);
    scaleBar.style.width = finalWidth + 'px';
    ...
    scaleBar.style.padding = '2px 6px';
...
function getRampColor(t) {
    ...
    let factor = (t - left) / (right - left);
    factor = Math.pow(factor, gammaValue);
...
function gaussianBlur(grid, nx, ny, sigma) {
    ...
    const result = [];
    for (let y = 0; y < ny; y++) result.push(new Array(nx).fill(0));
    const kernelSize = Math.max(1, Math.floor(sigma * 3));
    const kernel = [];
    let kernelSum = 0;
    ...
    for (let y = 0; y < ny; y++) {
        for (let x = 0; x < nx; x++) {
            let sum = 0;
            kernel.forEach(k => {
                const ny2 = Math.min(ny-1, Math.max(0, y + k.dy));
                const nx2 = Math.min(nx-1, Math.max(0, x + k.dx));
                sum += grid[ny2][nx2] * k.weight;
            });
            result[y][x] = sum;
        }
    }
...
function polygonPerimeter(points) {
    ...
    for (let i = 0; i < n; i++) {
        const j = (i + 1) % n;
        perim += Math.hypot(points[i].x - points[j].x, points[i].y - points[j].y);
    }
    return perim;
}
...
function redrawCurrentMode() {
    ...
    const iterations = smoothValue > 0 ? 1 : 0;
    ...
    if (mode === 'contour') {
        ...
        const blurred = gaussianBlur(transformed, currentNx, currentNy, 1.5);
        const flat = blurred.flat();
        ...
        contours.forEach(contour => {
            ...
            contour.coordinates.forEach(polygon => {
                polygon.forEach(ring => {
                    ...
                    if (useCurve) {
                        if (closed) {
                            finalPoints = chaikinSmoothClosed(points, 1);
                        } else {
                            finalPoints = chaikinSmooth(points, 1);
                        }
                    }
        ...
        console.log(`Додано path: ${pathCount} (згладжування Chaikin: ${iterations})`);
...
// Рендеринг Displacement з високоякісною інтерполяцією та гаммою
    ...
    for (let y = 0; y < targetSize; y++) {
        ...
        for (let x = 0; x < targetSize; x++) {
            ...
            if (val <= lowPosVal) {
                ...
                let t = (val - lowPosVal) / (highPosVal - lowPosVal);
                t = Math.pow(t, gammaValue);
...
smoothnessInput.value = 3;
smoothVal.textContent = '3';
...
gammaInput.addEventListener('input', function(e) {
    gammaVal.textContent = parseFloat(e.target.value).toFixed(2);
    gammaValue = parseFloat(e.target.value);
    redrawCurrentMode();
});
