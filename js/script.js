...
function redrawCurrentMode() {
    ...
    const iterations = smoothValue > 0 ? 1 : 0; // лише одна ітерація для уникнення петель
    ...
    if (mode === 'contour') {
        ...
        // Сильне гаусове розмиття для згладжування шуму
        ...
        let pathCount = 0;
        const minArea = 150;          // високий поріг площі
        const minPoints = 10;          // більше точок
        ...
        contours.forEach(contour => {
            ...
            contour.coordinates.forEach(polygon => {
                polygon.forEach(ring => {
                    ...
                    // Обчислюємо площу для замкнених контурів
                    if (closed) {
                        const area = polygonArea(points);
                        if (area < minArea) return; // пропускаємо дрібні петлі
                    }
                    
                    // Відкидаємо дуже короткі контури
...
// Встановлюємо значення за замовчуванням 3
