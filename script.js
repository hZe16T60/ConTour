...
// ========== Функції згладжування ліній (алгоритм Chaikin) ==========
function chaikinSmooth(points, iterations = 1) {
    ...
    for (let iter = 0; iter < iterations; iter++) {
        const newPoints = [];
        // Додаємо першу точку (вона залишається)
        newPoints.push(smoothed[0]);
        
        for (let i = 0; i < smoothed.length - 1; i++) {
            const p0 = smoothed[i];
            const p1 = smoothed[i + 1];
            // Обчислюємо дві нові точки, що ділять відрізок у співвідношенні 1:3 та 3:1
            const q = {
        ...
        // Якщо ламана замкнена (перша і остання близькі), потрібно додатково обробити
...
function redrawCurrentMode() {
    ...
    const iterations = Math.round(smoothValue * 2); // 0, 2, 4, 6 для значень 0..3
    
    if (mode === 'contour') {
        // Для лінійної мапи: трансформуємо сітку за допомогою контрастного розтягу
        const transformed = [];
        ...
        contours.forEach(contour => {
            ...
            contour.coordinates.forEach(polygon => {
                polygon.forEach(ring => {
                    ...
                    // Згладжуємо лінію, якщо потрібно
                    if (iterations > 0) {
                        points = chaikinSmooth(points, iterations);
                    }
                    ...
                    if (Math.hypot(first.x - last.x, first.y - last.y) < 1) d += ' Z';
        ...
        console.log(`Додано path: ${pathCount} (згладжування ліній: ${iterations} ітерацій)`);
    } else if (mode === 'displacement') {
        // Для дисплейсменту використовуємо оригінальну сітку, але збільшуємо роздільну здатність через інтерполяцію
...
function renderDisplacement(grid, nx, ny) {
    ...
    // Створюємо canvas високої роздільної здатності (наприклад, 1024x1024)
    ...
    for (let y = 0; y < targetSize; y++) {
        ...
        for (let x = 0; x < targetSize; x++) {
            ...
            // Білінійна інтерполяція значення висоти
            const v00 = grid[ny-1 - y1][x1]; // інвертуємо y для правильного відображення
    ...
    // Масштабуємо до 400x400 з високоякісною інтерполяцією
...
// Запобігання відкриттю при перетягуванні
